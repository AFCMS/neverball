...
{
    SDL_Event event = { SDL_EVENT_USER };
    event.user.code = code;
    SDL_PushEvent(&event);
}

/* Generated by Codex CLI 0.144.4 (GPT-5). */
void web_pause_main_loop(void)
{
    emscripten_pause_main_loop();
}

/* Generated by Codex CLI 0.144.4 (GPT-5). */
void web_resume_main_loop(void)
{
...
static int loop(void)
{
    ...
    while (d && SDL_PollEvent(&e))
    {
        ...
        case SDL_EVENT_MOUSE_MOTION :
            ...
            ax = ROUND(+e.motion.x * video.device_scale);
            ay = ROUND((-e.motion.y + video.window_h) * video.device_scale);
            dx = ROUND(+e.motion.xrel * video.device_scale);
            dy = ROUND((config_get_d(CONFIG_MOUSE_INVERT) ?
                        +e.motion.yrel : -e.motion.yrel) * video.device_scale);
...
static void step(void *data)
{
    ...
    if (running)
    {
        ...
        Uint64 dt = (now - mainloop->now);

        if (0 < dt && dt < 1000)
        {
            /* Step the game state. */

            st_timer(0.001f * (float) dt);

            /* Render. */

            hmd_step();
            st_paint(0.001f * (float) now);
            video_swap();
        }
