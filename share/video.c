...
{
    bool ok = SDL_SetWindowFullscreen(window, f != 0);
...
void video_set_window_size(int w, int h)
{
     ...
     *   3) triggers a SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED event, which updates our viewport/UI.
     */

    if (!SDL_SetWindowSize(window, w, h))
        log_printf("Failure to resize window (%s)\n", SDL_GetError());
    else if (!SDL_SyncWindow(window))
        log_printf("Failure to synchronize window resize (%s)\n", SDL_GetError());
}
...
int video_mode(int f, int w, int h)
{
    ...
    if (displays)
    {
        if (dpy < 0 || dpy >= display_count)
            dpy = 0;

        if (display_count > 0)
            display = displays[dpy];
    ...
    flags = SDL_WINDOW_OPENGL |
            (highdpi ? SDL_WINDOW_HIGH_PIXEL_DENSITY : 0) |
...
void video_swap(void)
{
    ...
    if (ticks > 1000)
    {
        ...
        double k = 1000.0 * frames / (double) ticks;
        double f = floor(k);
        double c = ceil (k);
